...
void Server :: executeCommands(int fd)
{
    ...
    void (Server::*cmds[])(Client &client) = {&Server::PASS, &Server::NICK, &Server::USER, &Server::JOIN, &Server::QUIT, &Server::INVITE, &Server::WHO, &Server::PRIVMSG, &Server::PART, &Server::KICK};
    std::string commands[] = {"PASS", "NICK", "USER", "JOIN", "QUIT", "INVITE", "WHO", "PRIVMSG", "PART", "KICK"};
    size_t i;

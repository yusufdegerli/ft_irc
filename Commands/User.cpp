...
{
    if (!client.getUsername().empty() && !client.getHostname().empty() && client.getLoggedStatus()) //if they are registers ones, quits and comes back, it should not re-add the same client
    {
    ...
    if (this->commands.size() == 5 && this->commands[2].compare("0") == 0 && this->commands[3].compare("*") == 0 && client.getUsername() == "")
    {
